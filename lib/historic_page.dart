...
class _HistoricoPageState extends State<HistoricoPage> {
  ...
  Future<void> _loadEnergyPrice() async {
    ...
    final snap = await FirebaseFirestore.instance
        ...
        .get();
    final price =
        (snap.data()?['settings']?['energyPrice'] as num?)?.toDouble() ??
        0.22;
                ...
                items: [
                  ...
                  ...devices.map((doc) {
                    ...
                    return DropdownMenuItem<String>(
                      value: doc.id,
                      child: Text((data['name'] as String?) ?? 'Sem nome'),
                    );
                  }),
                ],
                onChanged: (value) {
  ...
  Widget _buildRankingTab(String userId) {
    return StreamBuilder<List<DeviceStats>>(
      ...
      builder: (context, snapshot) {
        ...
        if (ranking.isEmpty) return const Center(child: Text('Sem dados'));
        return ListView.separated(
          padding: const EdgeInsets.all(16),
        ...
        for (final deviceDoc in devicesSnap.docs) {
          ...
          for (final doc in snap.docs) {
            ...
            allReadings.add(Reading(
              powerW: (data['powerW'] as num?)?.toDouble() ?? 0.0,
              timestamp:
  ...
  Stream<List<DeviceStats>> _getDeviceRankingStream(String userId) {
    ...
    return FirebaseFirestore.instance
      ...
      for (final deviceDoc in devicesSnapshot.docs) {
        ...
        final deviceName =
            (deviceData['name'] as String?) ?? 'Sem nome';
      ...
      final totalGeral = stats.fold(0.0, (sum, d) => sum + d.kwh);
      stats.sort((a, b) => b.kwh.compareTo(a.kwh));
      for (final device in stats) {
        device.percent =
            totalGeral > 0 ? (device.kwh / totalGeral * 100) : 0.0;
      }
      return stats;
    });
  }
  ...
  int _calculatePontos(double kwhAtual, double mediaMensal) {
    ...
    final poupancaKwh =
        (mediaMensal - kwhAtual).clamp(0.0, double.infinity);
    final percentReducao =
        (poupancaKwh / mediaMensal * 100).clamp(0.0, 100.0);
    return percentReducao.round();
  }
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle:
                  const Text('Partilhar ou guardar no dispositivo'),
              onTap: () {
  ...
  Future<void> _exportPdf(BuildContext context) async {
    ...
    try {
      ...
      for (final deviceDoc in devicesSnap.docs) {
        final deviceData = deviceDoc.data();
        final name = (deviceData['name'] as String?) ?? 'Sem nome';
      ...
      final mediaKwh =
          dailyStats.isEmpty ? 0.0 : totalKwh / dailyStats.length;
      ...
      pdf.addPage(pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Row(children: [
            _pdfStatBox('Total Consumido',
                '${totalKwh.toStringAsFixed(2)} kWh'),
            pw.SizedBox(width: 12),
            _pdfStatBox('Custo Estimado',
                '${custoTotal.toStringAsFixed(2)} €'),
            pw.SizedBox(width: 12),
            _pdfStatBox('Média Diária',
          ...
          if (dailyStats.isEmpty)
            pw.Text('Sem dados para o período selecionado.',
                ...
                ...dailyStats.asMap().entries.map((entry) {
                  ...
                  return pw.TableRow(
                    ...
                    children: [
                      ...
                      _pdfTableCell(
                          (d.kwh * _energyPrice).toStringAsFixed(2)),
                      _pdfTableCell(
          ...
          pw.Text(
            'Nota: O custo é calculado com base na tarifa de '
            '${_energyPrice.toStringAsFixed(2)} €/kWh.',
            style: const pw.TextStyle(
      ...
      await Printing.sharePdf(
        ...
        filename:
            'averis_consumo_${fmtDate(_dataInicio).replaceAll('/', '-')}'
            '_${fmtDate(_dataFim).replaceAll('/', '-')}.pdf',
      );
    } catch (e) {
      if (context.mounted) {
        Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            color != null ? color!.withOpacity(0.1) : Colors.blue.shade50,
...
class DailyChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxKwh = math.max(baselineKwh,
    ...
    for (int i = 0; i < dailyStats.length; i++) {
      ...
      final double x = dailyStats.length == 1
          ? size.width / 2
          : 40 + i * ((size.width - 60) / (dailyStats.length - 1));
      final y =
          size.height * 0.7 - (stat.kwh / maxKwh * size.height * 0.5);
      canvas.drawRect(Rect.fromLTWH(x - 15, y, 30, size.height * 0.7 - y),
          Paint()..color = Colors.blue.withOpacity(0.7));
      canvas.drawCircle(
          Offset(x, y), 5, Paint()..color = Colors.blue);
    }
  }
